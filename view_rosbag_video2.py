...
if __name__ == "__main__":
  ...
  bags = serialrosbags.Bags(datasets, ['/center_camera/image_color/compressed','/right_camera/image_color/compressed','/left_camera/image_color/compressed','/vehicle/steering_report'])
  while bags.has_data():
    ...
    if startsec == 0:
        ...
        if t.to_sec() > skipping:
            if topic in ['/center_camera/image_color/compressed','/right_camera/image_color/compressed','/left_camera/image_color/compressed']:
              blue = (255, 0, 0)
              print(topic, msg.header.seq, t-msg.header.stamp, msg.format, t)
            else:
            ...
            try: 
              if topic in ['/center_camera/image_color/compressed','/right_camera/image_color/compressed','/left_camera/image_color/compressed']:
                if topic == '/left_camera/image_color/compressed':
                  fstr = cStringIO.StringIO(msg.data)
                  imgleft = pygame.transform.scale(pygame.image.load(fstr), (320, 240))
                else:
                  if topic == '/center_camera/image_color/compressed':
                    fstr = cStringIO.StringIO(msg.data)
            ...
            draw_path_on(imgleft, 0, angle_steers*20, blue, -10)
            draw_path_on(imgcenter, 0, angle_steers*20, blue, -20)
            draw_path_on(imgright, 0, angle_steers*20, blue, 0)
